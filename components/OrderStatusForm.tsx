...
export default function OrderStatusForm() {
  ...
  return <div className="bg-[#111] border border-white/10 p-5 sm:p-7">
    <label className="block text-[#aaa] text-[10px] font-black tracking-[0.18em] uppercase mb-2">Order number</label>
    <input value={orderId} onChange={(event) => setOrderId(event.target.value)} placeholder="SR-XXXXXXXXXXXX" className="w-full bg-[#171717] border border-white/15 px-4 py-3 text-white mb-4 focus:border-[#E8500A] outline-none" />
    <label className="block text-[#aaa] text-[10px] font-black tracking-[0.18em] uppercase mb-2">Checkout email</label>
    <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="w-full bg-[#171717] border border-white/15 px-4 py-3 text-white mb-4 focus:border-[#E8500A] outline-none" />
