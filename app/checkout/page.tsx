...
function SummaryContent({ products, subtotal, discount, postage, total, cartMode, removeItem }: {
  ...
  return <div className="p-5 sm:p-6">
    <div className="divide-y divide-white/8 border-b border-white/8 mb-5">{products.map((product) => <div key={product.id} className="flex gap-3 py-4 first:pt-0"><div className="relative w-16 h-16 bg-[#1a1a1a] border border-white/8 shrink-0 overflow-hidden">{product.imageUrls?.[0] ? <Image src={product.imageUrls[0]} alt="" fill sizes="64px" className="object-cover" /> : <span className="h-full flex items-center justify-center text-[#999] text-[10px]">ITEM</span>}</div><div className="min-w-0 flex-1"><p className="text-white text-sm font-bold leading-snug">{websiteProductTitle(product.name)}</p><p className="text-[#d2d2d2] text-xs font-semibold leading-snug mt-1">{product.brand} · Size {productSizeLabel(product)}</p>{cartMode && <button type="button" onClick={() => removeItem(product)} className="min-h-8 text-red-300 hover:text-red-200 text-[11px] mt-1 underline">Remove</button>}</div><p className="text-white text-sm font-bold">£{product.price.toFixed(2)}</p></div>)}</div>
