...
function SummaryContent({ products, subtotal, discount, postage, total, cartMode, removeItem }: {
  ...
  return <div className="p-5 sm:p-6">
    <div className="divide-y divide-white/8 border-b border-white/8 mb-5">{products.map((product) => <div key={product.id} className="flex gap-3 py-4 first:pt-0"><div className="w-16 h-16 bg-[#1a1a1a] border border-white/8 shrink-0 overflow-hidden">{product.imageUrls?.[0] ? <img src={product.imageUrls[0]} alt="" className="w-full h-full object-cover" /> : <span className="h-full flex items-center justify-center text-[#777] text-[10px]">ITEM</span>}</div><div className="min-w-0 flex-1"><p className="text-white text-sm font-bold leading-snug">{product.name}</p><p className="text-[#888] text-[11px] mt-1">{product.brand} · Size {product.size}</p>{cartMode && <button type="button" onClick={() => removeItem(product)} className="text-red-300 hover:text-red-200 text-[11px] mt-2 underline">Remove</button>}</div><p className="text-white text-sm font-bold">£{product.price.toFixed(2)}</p></div>)}</div>
    <div className="space-y-2.5 mb-5 text-sm"><div className="flex justify-between"><span className="text-[#bbb]">Items</span><span className="text-white font-bold">£{subtotal.toFixed(2)}</span></div>{discount > 0 && <div className="flex justify-between text-emerald-300"><span>VINTAGE10 discount</span><span className="font-bold">−£{discount.toFixed(2)}</span></div>}<div className="flex justify-between"><span className="text-[#bbb]">UK tracked delivery</span><span className={`font-bold ${postage === 0 ? "text-emerald-300" : "text-white"}`}>{postage === 0 ? "FREE" : `£${postage.toFixed(2)}`}</span></div></div>
...
function CheckoutContent() {
  ...
  const discountedSubtotal = promotionApplied && products ? products.reduce((sum, product) => sum + Number((product.price * 0.9).toFixed(2)), 0) : subtotal;
  ...
  const applyPromotion = () => {
    if (promotionInput.trim().toUpperCase() !== "VINTAGE10") { setPromotionApplied(false); setPromotionError("That code is not recognised. Check the spelling and try again."); return; }
    setPromotionApplied(true); setPromotionError("");
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/create-checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ itemIds: products.map((product) => product.id), name, email, phone, address, promotionCode: promotionApplied ? "VINTAGE10" : "" }) });
      const data = await response.json();
  ...
  return <div className="min-h-screen bg-[#0a0a0a]"><Navbar /><div className="mt-[92px] max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    ...
    <div className="grid lg:grid-cols-[1fr_420px] gap-10 items-start"><div><p className="text-[#E8500A] text-[11px] font-black tracking-[0.25em] uppercase mb-3">Secure checkout</p><h1 className="text-white text-2xl sm:text-3xl font-black tracking-wider mb-2" style={{ fontFamily: "var(--font-playfair-display), serif" }}>Delivery details</h1><p className="text-[#999] text-sm mb-7">We only use these details to fulfil your order.</p>
      <form onSubmit={handleSubmit} className="space-y-4"><Field label="Full name" value={name} onChange={setName} placeholder="John Smith" autoComplete="name" required /><Field label="Email" value={email} onChange={setEmail} type="email" placeholder="john@email.com" autoComplete="email" required /><Field label="Phone" value={phone} onChange={setPhone} type="tel" placeholder="+44 7700 900000" autoComplete="tel" required /><Field label="Address line" value={addressLine} onChange={setAddressLine} placeholder="12 Example Street" autoComplete="address-line1" required /><div className="grid sm:grid-cols-2 gap-4"><Field label="Town or city" value={city} onChange={setCity} placeholder="London" autoComplete="address-level2" required /><Field label="Postcode" value={postcode} onChange={setPostcode} placeholder="SW1A 1AA" autoComplete="postal-code" required /></div>
        <div className="border border-white/10 p-4"><label htmlFor="promotion-code" className="block text-[#bbb] text-[11px] tracking-[0.14em] uppercase mb-2">Discount code</label><div className="flex gap-2"><input id="promotion-code" value={promotionInput} onChange={(event) => { setPromotionInput(event.target.value); setPromotionApplied(false); setPromotionError(""); }} placeholder="Enter code" className="min-w-0 flex-1 bg-[#1a1a1a] border border-white/15 text-white text-base px-4 py-3 outline-none focus:border-[#E8500A] uppercase" /><button type="button" onClick={applyPromotion} className="border border-white/20 text-white px-4 text-xs font-bold uppercase hover:border-[#E8500A]">Apply</button></div>{promotionApplied && <p className="text-emerald-300 text-xs mt-2">10% discount applied — you save £{discount.toFixed(2)}.</p>}{promotionError && <p className="text-red-300 text-xs mt-2">{promotionError}</p>}</div>
