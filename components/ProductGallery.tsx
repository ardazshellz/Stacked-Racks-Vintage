...
export default function ProductGallery({ images, name, productId, price }: { images: string[]; name: string; productId: string; price: number }) {
  ...
  if (!images.length) {
    return <div className="aspect-[3/4] bg-[#151515] border border-white/10 flex items-center justify-center text-[#999]">Photo coming soon</div>;
  }

  return (
    <div>
      <button type="button" onClick={() => setZoomed(true)} className="relative block w-full aspect-[3/4] bg-[#111] border border-white/10" aria-label="Enlarge product photo">
      ...
      {images.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Product photos">
